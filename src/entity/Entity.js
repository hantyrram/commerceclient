...
class Entity{
 ...
 /**
  * @type {String} 
  * @description The unique identifier of the entity. This getter MUST be overriden by Entities, with identifiers
  * other than _id. The save method uses this property to update when present,or create when not.
  * 
  */
 get identifier(){
    return '_id';
  ...
  * If instance has no identifier, then this method will add the entity. If identifier is present it will send an UPDATE 
  * request.
  */
 async save(){
  if(!this.identifier){
  ...
  try {
   let response = await axios.update(apis(this.updateActionName()),this);
